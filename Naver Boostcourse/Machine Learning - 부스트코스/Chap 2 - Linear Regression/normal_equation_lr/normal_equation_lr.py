...
class LinearRegression(object):
    ...
    def fit(self, X, y):
        self._new_X = np.array(X)   # numpy array 형태로 변형하여 새로운 변수 new_X에 저장 (입력이 어떻게 들어올 지 모르기 때문)
        y = y.reshape(-1,1)         # matrix 형태로 변형
        
        if self.fit_intercept:      # X 매트릭스 맨 앞 열에 "1"로 이루어진 column vector 추가
            intercept_vector = np.ones([len(self._new_X),1])  # "1" 300개 추가
            self._new_X = np.concatenate(
                (intercept_vector,self._new_X), axis=1)
            
        weights = np.linalg.inv(  # 역함수, transpose 등등으로 선형회귀 공식 구현 (XT*X)^-1*(XT*y)
                self._new_X.T.dot(self._new_X)).dot(
                                                self._new_X.T.dot(y)).flatten()  # n by 1 데이터를 1-dimension으로 flatten
