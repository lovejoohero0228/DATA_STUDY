...
/* CASE WHEN 함수 사용시 주의점 : 순차적 IF ELSE처럼 작용 */
...
/****** 회원 분석용 데이터마트 생성 ( 회원 구매정보 + 연령대 + 선호 카테고리 임시테이블 ) ******/
...
/***** 데이터 마트의 구매자 비중(%)에 오류는 없는가? *****/
/* CUSTOMER 테이블 기준, SALES 테이블 구매 회원번호 LEFT JOIN 결함 */
SELECT *
FROM CUSTOMER AS A
LEFT JOIN (
