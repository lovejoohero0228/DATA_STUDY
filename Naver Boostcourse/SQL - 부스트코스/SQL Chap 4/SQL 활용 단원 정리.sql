...
SELECT SUM(A.SALES_QTY * B.PRICE) / COUNT(A.ORDER_NO) AS 1회평균구매금액
FROM SALES AS A
...
FROM (
		SELECT MEM_NO
			   ...
			   , ROW_NUMBER() OVER (ORDER BY SUM(SALES_QTY) DESC) AS 구매수량_순위
			FROM SALES
...
DELIMITER //
CREATE PROCEDURE BDAY(IN MONTH_A INT, MONTH_B INT)
BEGIN
	 SELECT *
     FROM CUSTOMER
     WHERE MONTH(BIRTHDAY) BETWEEN MONTH_A AND MONTH_B;
END //
DELIMITER ;
